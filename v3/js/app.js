...
priceInput.addEventListener("input", () => {
  ...
  // MATCH só inicia ao clicar em "Buscar oportunidade".
  resetMatchFlow();
  updateBuscarState();
});
...
// ===== Buscar oportunidade (gatilho único) =====
function isValidModel(){
  const v = (modelInput ? modelInput.value : "").trim();
  return v.length >= 2;
}
...
function updateBuscarState(){
  if(!buscarOpp) return;
  buscarOpp.disabled = !(isValidModel() && isValidBudget());
}
function resetMatchFlow(){
  // cancela timers e estados visuais
  cancelRadarFlow();
  clearInterval(stageTimer);
  stageTimer = null;
  currentMatch = null;
  ...
  matchBtn.textContent = "Ver MATCH";
  hideMatchBar();
  // RP MATCH volta ao estado cinza
  if(radarPill){
    radarPill.classList.remove("ready","scanning");
  }
}
function setRPState(state){
  if(!radarPill) return;
  radarPill.classList.remove("ready","scanning");
  if(state === "ready") radarPill.classList.add("ready");
  if(state === "scanning") radarPill.classList.add("scanning");
  // idle: nenhuma classe
}

// Model input changes should only enable button, not start MATCH
if(modelInput){
...
// Price blur keeps formatting, but updates button too
priceInput.addEventListener("blur", () => {
  updateBuscarState();
});

function scheduleMatch(){ /* desativado: MATCH inicia apenas por clique */ }
...
// ===== MATCH por clique (modelo + preço) =====
function normalizeQuery(q){
  return (q || "").toLowerCase().trim();
}
function matchesModel(offer, q){
  if(!q) return true;
  const t = (offer.title || "").toLowerCase();
  return t.includes(q);
}
function isLocal(offer){
  return offer.distanceLabel === "Muito perto" || offer.distanceLabel === "Perto";
}
function chooseMatchByModel(budget, query){
  const q = normalizeQuery(query);
  const pool = offers.filter(o => matchesModel(o, q));
  if(!pool.length) return null;

  const below = pool.filter(o => o.price <= budget);
  const candidates = below.length ? below : pool.slice();

  const ranked = candidates
    .map(o => ({ o, score: computeScore(o, budget) }))
    .sort((a,b) => {
      const aLV = (a.o.verified && isLocal(a.o)) ? 1 : 0;
      const bLV = (b.o.verified && isLocal(b.o)) ? 1 : 0;
      if(aLV !== bLV) return bLV - aLV;
      // Se ambos são local+verificado, prioriza menor preço
      if(aLV === 1 && bLV === 1 && a.o.price !== b.o.price) return a.o.price - b.o.price;
      return b.score - a.score;
    });

  // MATCH raro: exige um score mínimo quando existir pool do modelo
  const top = ranked[0];
  if(!top) return null;
  const THRESHOLD = 62; // ajuste fino depois
  if(top.score < THRESHOLD) return null;
  return top;
}

function showPriceGuidance(range, kind){
  if(!priceGuidance) return;
  const [min,max] = range;
  const budget = Number(priceInput.dataset.value || 0);

  const msg = kind === "low"
    ? `Para uma compra segura, valores entre <strong>R$ ${min}</strong> e <strong>R$ ${max}</strong> são mais comuns para este modelo.`
    : kind === "high"
      ? `Seu valor está acima da média. Faixa comum: <strong>R$ ${min}–${max}</strong>. Você pode economizar mantendo segurança.`
      : `Esse valor pode reduzir as chances de encontrar rapidamente. Faixa comum: <strong>R$ ${min}–${max}</strong>.`;

  priceGuidance.innerHTML = `
    <div>${msg}</div>
    <div class="actions">
      <button type="button" id="useSuggested">Usar sugestão</button>
      <button type="button" id="keepValue">Manter meu valor</button>
    </div>
  `;
  priceGuidance.style.display = "block";

  const useBtn = $("#useSuggested", priceGuidance);
  const keepBtn = $("#keepValue", priceGuidance);

  if(useBtn){
    useBtn.addEventListener("click", () => {
      // usa o limite mais coerente (se baixo -> min; se alto -> max; caso médio -> min)
      const newVal = (kind === "high") ? max : min;
      priceInput.value = formatPtNumber(newVal);
      priceInput.dataset.value = String(newVal);
...
// Orientação de preço: mock de faixa por modelo (substituir por base real depois)
function getMarketRange(query){
  const q = normalizeQuery(query);
  const ranges = {
    "iphone 11": [2200,2400],
    "iphone 12": [2600,2900],
    "iphone 13": [3000,3400],
    "iphone 14": [3600,4200],
    "galaxy s23": [3000,3400],
    "galaxy s22": [2300,2800]
  };
  return ranges[q] || null;
}

function startMatchClickFlow(){
  if(!isValidModel() || !isValidBudget()) return;
  resetMatchFlow();

  const query = modelInput.value.trim();
  const budget = Number(priceInput.dataset.value || 0);

  // orientação de preço (não bloqueia)
  const range = getMarketRange(query);
  if(range){
    const [min,max] = range;
    if(budget < min * 0.9) showPriceGuidance(range, "low");
    else if(budget > max * 1.1) showPriceGuidance(range, "high");
    else if(budget < min) showPriceGuidance(range, "mid");
  }

  // RP MATCH muda de estado (verde claro)
  setRPState("ready");

  // Tempo: 5s -> scanning; + 10s para tentar mostrar match (variação 8–12 fica p/ depois)
  silentTimer = setTimeout(() => {
    setRPState("scanning");

    radarTimer = setTimeout(() => {
      const chosen = chooseMatchByModel(budget, query);

      if(!chosen){
        // Sem frustração: mantém radar ativo indefinidamente
        // Mantém RP MATCH piscando e mostra mensagem suave
        if(priceGuidance && priceGuidance.style.display !== "block"){
          priceGuidance.innerHTML = `
            <div><strong>Radar RePhone ativo.</strong> Vamos monitorar oportunidades compatíveis e avisar quando surgir uma oferta ideal.</div>
            <div style="margin-top:.35rem; font-weight:700; color: rgba(15,23,42,.65);">Dica: você pode ajustar o valor para aumentar as chances, ou expandir a distância.</div>
          `;
          priceGuidance.style.display = "block";
        }
        return;
      ...
      // Exibe barra do MATCH como evento
      matchTitle.textContent = "MATCH encontrado ✦";
      matchSub.textContent = "Uma oportunidade compatível apareceu para você.";
      matchBtn.disabled = false;
      matchBtn.textContent = "Ver MATCH";
      resetProgress();
      matchProg.style.width = "100%";
      ...
      // Para o blinking do círculo quando vira barra
      setRPState("idle");
      render();
    }, 10000);
...
if(buscarOpp){ buscarOpp.addEventListener("click", startMatchClickFlow); }
updateBuscarState();

// Fechar barra do MATCH
const matchClose = $("#matchClose");
if(matchClose){
  matchClose.addEventListener("click", () => {
    // fecha e volta pro estado inicial
    hideMatchBar();
    currentMatch = null;
    hiddenOfferId = null;
    matchBtn.disabled = true;
